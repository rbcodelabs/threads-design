...
- `/design spec: the pricing page` adds a toggleable redline overlay to the existing design without changing it. Press `S` or click the floating "Show spec" button to reveal spacing, type, and color-token annotations, in both the desktop and mobile layouts. Spec only works in a thread that already has a design; it cannot start a new thread.

The colon is required, so `/design states of the union dashboard` is an ordinary brief. In a thread that already has a design, a bare `/design wireframe:`, `/design states:`, `/design variations:`, or `/design spec:` runs that mode against the existing design. `EnterDesignMode` accepts the same modes through its optional `mode` argument. In an Agent Threads host version that supports argument-completion suggestions, typing `/design ` offers the mode keywords as autocomplete.
